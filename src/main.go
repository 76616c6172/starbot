...
// Is called by AddHandler every time a new message is created - in ANY channel the bot has access to
func scan_message(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	case CPL_CLIPS_CHANNEL_ID: // monitor cpl clips channel and save messages with twitch URLS
...
func main() {
	...
	if len(os.Args) != 2 { // Check to make sure a bot auth token was supplied on startup
	...
	logfile, err := os.OpenFile("log.html", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666) // open file for match report logging
	if err != nil {
		checkError(err)
	}
	...
	load_persistent_internal_data_structures() // load persistent data into memory

	dg, err := discordgo.New("Bot " + TOKEN) // initiate discord session through the discord API
	if err != nil {
		fmt.Println("Error creating discord session", err)
	}

	dg.AddHandler(scan_message)                // register scan_message as a callback func for message events
	dg.Identify.Intents = discordgo.IntentsAll // receive all events on the server

	err = dg.Open() // establish the discord session
	if err != nil {
	...
	// just testing
	test := mapWebUserIdToPlayer
	fmt.Println(test)

	fmt.Println("Bot is running..")
	sc := make(chan os.Signal, 1) // keep running until exit signal is received..
	signal.Notify(sc, syscall.SIGINT, syscall.SIGTERM, os.Interrupt, os.Kill)
	<-sc

	dg.Close() // gracefully close down the Discord session on exit
